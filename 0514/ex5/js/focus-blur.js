...
var el = document.getElementById('username');       // Username 입력 필드를 찾음
var elMsg = document.getElementById('feedback');    // 미시지를 저장할 요소를 찾음

// username 입력 필드가 포커스를 받거나 잃으면 위의 함수를 호출하도록 한다.
el.addEventListener('focus', tipUsername, false);   // focus 이벤트는 tipUsername() 함수를 호출한다.
el.addEventListener('blur', checkUsername, false);  // blur 이벤트는 checkUsername() 함수를 호출한다.